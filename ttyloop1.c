 ...
 * Command  :  "ttyloop1"                                    *
 * Input    :    NONE.                                       *
 * Description:  This test exercises the kernel ttyS1 device *
...
int main( int argc, char ** argv )
{
  ...
  fd_serial = open("/dev/ttyS1", O_RDWR | O_NONBLOCK | O_NOCTTY);
  if (fd_serial <= 0)
    {
  ...
  // Get current settings for /dev/ttyS1
  ...
  sprintf(test_buff_out, "Hello from ttyS1\n");
