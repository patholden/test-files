...
main( int argc, char ** argv )
{
  ...
  memset(char_curr, 0, sizeof(char_curr));
  memset(char_last, 0, sizeof(char_last));
  while (1)
    {
      for (sample = 0; sample <= SAMPLERATE; sample++)
	{
	  ...
	  char_curr[0] = inb(TFPORTRH);
	  char_curr[1] = inb(TFPORTRL);
	  if (char_curr[0] != char_last[0])
	    syslog(LOG_NOTICE, "TFPORTRH change: last %x, curr %x",char_last[0], char_curr[0]);
	  if (char_curr[1] != char_last[1])
	    syslog(LOG_NOTICE, "TFPORTRL change: last %x, curr %x",char_last[1], char_curr[1]);
	  char_last[0] = char_curr[0];
	  char_last[1] = char_curr[1];
	}
    }
   exit(EXIT_SUCCESS);
}
